...
angular.module('rest').factory('tunnelService', ['$injector',
    ...
    service.uploadToStream = function uploadToStream(tunnel, stream, file,
        ...
        var uploadedSize = 0;
        var lastFragmentSize = 0;
        var lastFragmentTime = 0;
        ...
        const uploadChunk = (chunk, offset) => {
            ...
            xhr.onreadystatechange = function uploadStatusChanged() {
                ...
                if (xhr.status >= 200 && xhr.status < 300) {
                    ...
                    if (uploadedSize == chunk.size) {
                        lastFragmentTime = Math.floor(Date.now() / 1000);
                        const dt = new Date(lastFragmentTime * 1000).toUTCString();
                        console.log(">>> Started uploading at: " + dt);
                    } else {
                        if (lastFragmentSize + 1000000000 < uploadedSize) {
                            const diff = uploadedSize - lastFragmentSize;
                            const now = Math.floor(Date.now() / 1000);
                            const started = new Date(lastFragmentTime * 1000).toUTCString();
                            const finished = new Date(now * 1000).toUTCString();

                            console.log(">>> Uploaded: " + diff +
                                " (b); started: " + started +
                                "; finished: " + finished +
